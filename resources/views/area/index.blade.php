...
@section('content')
    ...
    <div class="content-card">
        ...
        <div class="header-container">
            <div>
                <h3 class="section-title">Catálogo de Áreas de Intervención</h3>
                <p class="section-subtitle" style="color: var(--color-text-medium); margin-top: 0.5rem;">
                    Gestión de áreas geográficas de intervención del sistema
                </p>
            </div>
        ...
        <div class="stats-grid" style="margin-bottom: 2rem;">
            <div class="stat-card card-border-green">
                <div class="card-content-header">
                    <span class="card-label">Total Áreas</span>
                ...
                <div class="card-value">{{ $areas->count() }}</div>
            ...
            <div class="stat-card card-border-indigo">
                ...
                <div class="card-value">{{ $areas->pluck('departamento')->unique()->count() }}</div>
                <p class="card-description">Diferentes departamentos</p>
            </div>
        </div>
            ...
            @if ($areas->isEmpty())
                <div class="empty-state">
                    ...
                    <p class="empty-text" style="margin-top: 1rem; font-size: 1.125rem; color: var(--color-text-medium);">
                        No hay áreas de intervención registradas en el sistema.
                        <br>
                        <a href="{{ route('area.create') }}" class="btn btn-primary" style="margin-top: 1rem;">
                        ...
                        @foreach ($areas as $area)
                            <tr>
                                <td style="font-weight: 600; color: var(--color-primary-dark);">
                                    <span class="badge" style="background: var(--color-primary-light); color: var(--color-primary-dark); padding: 0.25rem 0.75rem; border-radius: 0.5rem; font-size: 0.75rem; font-weight: 700;">
                                        {{ $area->codigo_area }}
                                    </span>
                                </td> 
                                <td style="font-weight: 600; color: var(--color-text-dark);">
                                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                                        <div style="width: 2rem; height: 2rem; background: linear-gradient(135deg, var(--color-info) 0%, var(--color-blue) 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 0.875rem;">
                                            {{ substr($area->nombre_area, 0, 1) }}
                                        </div>
                                        {{ $area->nombre_area }}
                                    </div>
                                </td>
                                ...
                                <td class="actions-cell">
                                    <div class="action-buttons">
                                        ...
                                        <button type="button" 
                                                onclick="openDeleteModal('{{ route('area.destroy', $area->codigo_area) }}', '{{ addslashes($area->nombre_area) }}')"
    ...
    <div id="deleteModal" class="modal-backdrop">
        <div class="modal-content">
            ...
            <p class="modal-text" style="text-align: center; color: var(--color-text-medium); line-height: 1.6; margin-bottom: 2rem;">
                ...
                <strong style="color: var(--color-text-dark);" id="areaName"></strong>?
                <br>
                <small style="color: var(--color-text-light); font-size: 0.875rem;">
                    Esta acción no se puede deshacer y podría afectar a fichas de beneficiarios asociadas.
                </small>
            </p>
    ...
    <script>
        ...
        function openDeleteModal(actionUrl, nombreArea) {
            deleteForm.action = actionUrl;
            areaName.textContent = nombreArea;

...
<div class="content-card">
    ...
    <div class="header-container">
        <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <div style="display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
                ...
                <h3 class="section-title">Editar Sesión</h3>
            </div>
            <div style="display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
    ...
    <div style="background: var(--color-bg-light); padding: 1.5rem; border-radius: var(--border-radius); border: 1px solid var(--color-border); margin-bottom: 2rem;">
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <div>
                <h5 style="font-family: var(--font-heading); font-weight: 600; color: var(--color-text-dark); margin-bottom: 0.25rem;">Editando sesión actual:</h5>
    ...
    <form action="{{ route('sesion.update', $sesion->id_sesion) }}" method="POST">
        ...
        <div class="form-grid">
            ...
            <div class="form-column" style="background: var(--color-bg-light); padding: 1.5rem; border-radius: var(--border-radius); border: 1px solid var(--color-border);">
                ...
                {{-- Campo Nro. Sesión (BLOQUEADO - READONLY) --}}
                <div class="form-group">
                    <label for="nro_sesion" class="form-label">
                        Número de Sesión <span style="font-weight: normal; color: var(--color-text-light); font-size: 0.85rem;">(No modificable)</span>
                    </label>
                    <div style="position: relative;">
                        <input type="number" 
                               id="nro_sesion" 
                               name="nro_sesion" 
                               value="{{ old('nro_sesion', $sesion->nro_sesion) }}" 
                               class="form-input form-disabled"
                               readonly 
                               style="background-color: #e9ecef; cursor: not-allowed; padding-left: 2.5rem;">
                        <svg style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); width: 1rem; height: 1rem; color: var(--color-text-light);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                    </div>
                </div>
                
                {{-- Campo Actividad (BLOQUEADO - DISABLED) --}}
                <div class="form-group">
                    <label for="actividad_nombre" class="form-label">
                        Actividad Relacionada <span style="font-weight: normal; color: var(--color-text-light); font-size: 0.85rem;">(No modificable)</span>
                    </label>
                    <div style="position: relative;">
                        <input type="text" 
                               id="actividad_nombre" 
                               value="{{ $sesion->actividad->nombre }}" 
                               class="form-input form-disabled" 
                               disabled
                               style="background-color: #e9ecef; cursor: not-allowed; padding-right: 2.5rem;">
                        <svg style="position: absolute; right: 1rem; top: 50%; transform: translateY(-50%); width: 1.25rem; height: 1.25rem; color: var(--color-text-light);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                    </div>
                    {{-- IMPORTANTE: Campo oculto para enviar el ID aunque el visual esté disabled --}}
                    <input type="hidden" name="id_actividad" value="{{ $sesion->id_actividad }}">
                </div>
            </div>
            ...
            <div class="form-column" style="background: var(--color-bg-light); padding: 1.5rem; border-radius: var(--border-radius); border: 1px solid var(--color-border);">
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           id="fecha" 
                           name="fecha" 
                           value="{{ old('fecha', \Carbon\Carbon::parse($sesion->fecha)->format('Y-m-d')) }}" 
                ...
                <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div>
                        ...
                        <input type="time" 
                               id="hora_inicio" 
                               name="hora_inicio" 
                               value="{{ old('hora_inicio', \Carbon\Carbon::parse($sesion->hora_inicio)->format('H:i')) }}" 
                    ...
                    <div>
                        ...
                        <input type="time" 
                               id="hora_fin" 
                               name="hora_fin" 
                               value="{{ old('hora_fin', \Carbon\Carbon::parse($sesion->hora_fin)->format('H:i')) }}" 
                ...
                <div id="duracion-indicator" style="background: rgba(30, 136, 229, 0.05); padding: 0.75rem; border-radius: var(--border-radius); border: 1px solid rgba(30, 136, 229, 0.2); margin-top: 1rem;">
        ...
        <div style="background: var(--color-bg-light); padding: 1.5rem; border-radius: var(--border-radius); border: 1px solid var(--color-border); margin: 2rem 0;">
            ...
            <div class="form-group">
                <label for="tema" class="form-label">
                    <span style="color: var(--color-danger);">*</span> Tema de la Sesión
                </label>
                <input type="text" 
                       id="tema" 
                       name="tema" 
                       ...
                       placeholder="Ej: Introducción a la Inclusión Educativa, Taller de Habilidades Sociales..."
                       required 
                       maxlength="150">
                @error('tema')
                    <div class="form-error-message">
                ...
                <div style="font-size: 0.75rem; color: var(--color-text-light); margin-top: 0.25rem;">
                    Máximo 150 caracteres. <span id="tema-counter">0/150</span>
                </div>
            </div>
        </div>
        ...
        <div style="background: #fff3cd; padding: 1.5rem; border-radius: var(--border-radius); border: 1px solid #ffeaa7; margin: 2rem 0;">
            <div style="display: flex; align-items: flex-start; gap: 1rem;">
                ...
                <div>
                    <h5 style="font-family: var(--font-heading); font-weight: 600; color: #856404; margin-bottom: 0.5rem;">Actualización de sesión</h5>
                    <p style="color: #856404; font-size: 0.875rem; margin: 0;">
                        Los cambios realizados aquí se reflejarán inmediatamente en el sistema. 
                        Verifique que la fecha y horario actualizados sean coherentes con la actividad seleccionada.
                    </p>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Contador de caracteres para el tema
    const temaInput = document.getElementById('tema');
    const temaCounter = document.getElementById('tema-counter');
    
    function updateTemaCounter() {
        const length = temaInput.value.length;
        temaCounter.textContent = `${length}/150`;
        
        if (length > 140) {
            temaCounter.style.color = 'var(--color-warning)';
            temaCounter.style.fontWeight = '600';
        } else if (length > 120) {
            temaCounter.style.color = 'var(--color-yellow)';
            temaCounter.style.fontWeight = '500';
        } else {
            temaCounter.style.color = 'var(--color-text-light)';
            temaCounter.style.fontWeight = 'normal';
        }
    }

    temaInput.addEventListener('input', updateTemaCounter);
    ...
    // Capitalizar automáticamente el tema
    ...
    // Mostrar alerta si hay cambios sin guardar
